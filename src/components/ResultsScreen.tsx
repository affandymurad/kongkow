...
function PaymentInfo({ dest }: { dest: Destination }) {
  ...
  if (venue_type === "public" && !has_entrance_fee) {
    return (
      <div className="w-full p-2.5 rounded-xl text-center bg-[#E1F5EE] text-[#0F6E56] font-extrabold text-xs border border-[#1D9E75]/20 select-none">
    ...
    return (
      <div className="w-full rounded-xl border overflow-hidden select-none">
        ...
        <div className={`px-3 py-2 flex items-start gap-2 ${has_entrance_fee ? "bg-[#FEF9EC] border-b border-amber-200/60" : "bg-[#E1F5EE] border-b border-[#1D9E75]/20"}`}>
          ...
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest font-mono text-amber-700">
              {has_entrance_fee ? "Tiket Masuk Berbayar" : "Tiket Masuk Gratis"}
            </p>
            <p className="text-[11px] font-bold text-[#18181A] leading-snug mt-0.5">
              {has_entrance_fee
        ...
        {has_entrance_fee && (
          <div className="px-3 py-2 bg-[#F0F4FA] flex items-start gap-2">
            ...
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest font-mono text-[#1B56B1]">
                Bayar Tiket Via
              </p>
              <p className="text-[11px] font-bold text-[#1D3A6F] leading-snug mt-0.5">
                {payment_methods}
              </p>
            </div>
          </div>
  ...
  if (venue_type === "street") {
    return (
      <div className="w-full p-2.5 rounded-xl bg-[#FAFAF8] border border-stone-200 flex items-center gap-2 select-none">
        ...
        <div>
          <p className="text-[9px] font-black uppercase tracking-widest font-mono text-stone-500">
            Metode Bayar
          </p>
          <p className="text-[11px] font-bold text-[#18181A] leading-none mt-0.5">
            {payment_methods}
          </p>
        </div>
      </div>
    ...
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden bg-[#F6F5F1] animate-fadeIn">
      ...
      <div className="bg-white border-b border-stone-200/80 px-4 py-3.5 shrink-0 flex justify-between items-center gap-3">
        <div className="space-y-1 min-w-0 flex-1">
          <h2 className="text-[14px] font-extrabold text-[#18181A] leading-tight flex items-start gap-1.5 font-sans">
            <Sparkles size={15} className="text-[#1D9E75] shrink-0 mt-0.5" />
            <span className="break-words">{headline}</span>
          </h2>
          <p className="text-[11px] font-medium text-[#5C5B57] font-sans leading-normal break-words whitespace-normal">
            {subHeadline}
          </p>
        </div>
        <button
          type="button"
          onClick={onShare}
          className="py-2.5 px-3 bg-[#E1F5EE] border border-[#1D9E75]/30 text-[#0F6E56] rounded-xl font-bold flex items-center gap-1.5 hover:bg-[#9FE1CB]/30 transition text-[11px] active:scale-95 shrink-0"
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 pb-32">
        {error ? (
          <div className="bg-[#FEF0ED] border border-[#E8381A]/30 rounded-2xl p-4.5 text-center space-y-3.5">
            <p className="text-xs text-[#E8381A] font-bold leading-relaxed">{error}</p>
            <button
              onClick={onRetry}
              className="bg-[#E8381A] text-white font-bold text-xs px-4 py-2 rounded-xl hover:opacity-90 active:scale-95 transition"
            >
              Coba Gaskeun Ulang
            </button>
          </div>
          ...
          <div className="text-center py-10 space-y-2">
            ...
            <p className="text-xs text-[#5C5B57] font-semibold">
              Sepertinya tidak ada kecocokan di lokasi tersebut.
            </p>
            <button
              onClick={onReset}
              className="text-xs bg-[#1D9E75] text-white px-4 py-2 rounded-xl font-bold"
            >
              Ganti Kriteria
            </button>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="h-28 bg-[#F6F5F1] relative flex items-center justify-center font-serif text-5xl select-none">
                    ...
                    {/* Distance pill */}
                    <span className="absolute bottom-3 left-3 bg-black/60 text-white font-mono text-[10px] font-bold px-2 py-0.5 rounded-full">
                      🚗 {dest.distance}
                    </span>
                    {/* Venue type badge */}
                    <span className={`absolute top-2.5 right-2.5 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border font-mono ${badge.cls}`}>
                      {badge.label}
                    </span>
                  </div>
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <div className="space-y-1 mb-2.5">
                      <h3 className="text-base font-extrabold text-[#18181A] leading-tight tracking-tight font-sans">
                        {dest.name}
                      </h3>
                      <p className="text-[11px] font-semibold text-[#0F6E56] font-mono uppercase tracking-wider flex items-center gap-1">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      <span className="text-[10px] font-semibold font-mono text-[#5C5B57] bg-[#F6F5F1] px-2.5 py-0.5 rounded-full border border-stone-200/40">
                        ⌛ {dest.duration}
                      </span>
                      <span className="text-[10px] font-semibold font-mono text-[#5C5B57] bg-[#F6F5F1] px-2.5 py-0.5 rounded-full border border-stone-200/40">
                        💵 {dest.price_range}
                      ...
                      {dest.tags?.slice(0, 3).map((tag, tIdx) => (
                        <span
                          key={tIdx}
                          className="text-[10px] uppercase font-bold font-mono text-[#0F6E56] bg-[#E1F5EE] px-2.5 py-0.5 rounded-full"
                        >
                          #{tag}
                        </span>
                      ))}
                    ...
                    <p className="text-xs text-[#5C5B57] leading-relaxed mb-3.5 font-sans font-normal">
                      {dest.desc}
                    </p>

                    {/* Tip box */}
                    <div className="p-3.5 bg-[#E1F5EE] rounded-xl border border-[#1D9E75]/15 mb-4">
                      <p className="text-[10px] font-black uppercase text-[#0F6E56] tracking-widest font-mono mb-1">
                        💡 Tip rahasia bestie
                      </p>
                      <p className="text-xs text-[#18181A] italic leading-relaxed font-medium">
                        "{dest.tip}"
                      </p>
                    </div>
                    ...
                    <div className="mt-auto space-y-2">
                      ...
                      <div className="grid grid-cols-12 gap-1.5 pt-1.5 border-t border-stone-100">
                        <button
                          ...
                          className="col-span-4 p-2 bg-white hover:bg-stone-50 border border-stone-200 text-[#5C5B57] hover:text-[#18181A] rounded-xl text-[11px] font-bold transition flex items-center justify-center gap-1 shrink-0 cursor-pointer"
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="col-span-8 p-2 bg-[#0F6E56] hover:bg-[#084A39] text-white text-[11px] font-black rounded-xl text-center flex items-center justify-center gap-1 transition active:scale-95 shadow-inner"
                        >
      ...
      <footer className="flex-shrink-0 px-4 py-3.5 border-t border-stone-200/80 bg-white/95 backdrop-blur-md z-20">
        <button
          type="button"
          onClick={onReset}
          className="w-full py-3 px-5 rounded-xl text-white bg-[#0F6E56] hover:bg-[#084A39] font-extrabold text-xs flex items-center justify-center gap-2 shadow-xs transition active:scale-95 cursor-pointer uppercase tracking-wider"
        >
